...
func TestListSkillTemplates(t *testing.T) {
	...
	if len(templates) == 0 {
		t.Fatal("expected at least one skill template")
	}
	...
	if !names["WIKI_SCHEMA.md"] {
		t.Errorf("expected WIKI_SCHEMA.md, got %v", names)
	}
	if !names["AGENTS.md"] {
		t.Errorf("expected AGENTS.md, got %v", names)
	}
	if !names["opencode.md"] {
		t.Errorf("expected opencode.md, got %v", names)
	}
	if !names["claude-code.md"] {
		t.Errorf("expected claude-code.md, got %v", names)
	}
	if !names["codex-cli.md"] {
		t.Errorf("expected codex-cli.md, got %v", names)
	}
	if !names["generic.md"] {
		t.Errorf("expected generic.md, got %v", names)
	}
}
...
func TestGetSkillTemplate(t *testing.T) {
	t.Run("existing template", func(t *testing.T) {
		...
		if tmpl.Name != "WIKI_SCHEMA.md" {
			t.Errorf("name = %q, want %q", tmpl.Name, "WIKI_SCHEMA.md")
		}
		if tmpl.Target != "reference" {
			t.Errorf("target = %q, want %q", tmpl.Target, "reference")
		}
		if tmpl.Content == "" {
	...
	t.Run("existing template AGENTS.md", func(t *testing.T) {
		tmpl, err := GetSkillTemplate("AGENTS.md")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if tmpl.Name != "AGENTS.md" {
			t.Errorf("name = %q, want %q", tmpl.Name, "AGENTS.md")
		}
	...
	t.Run("missing template", func(t *testing.T) {
		_, err := GetSkillTemplate("nonexistent.md")
		if err == nil {
